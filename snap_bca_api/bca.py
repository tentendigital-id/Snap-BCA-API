...
class BCA_SNAP:
    def __init__(
        self, client_id, client_secret, private_key, channel_id, partner_id, host="https://sandbox.bca.co.id", debug=False
    ):
    ...
    def __getRandomInt(self):
        random_number = "".join([str(random.randint(0, 9)) for _ in range(16)])  # nosec
    ...
    def __getSnapSymetricSignature(self, method, path, access_token, timestamp, request_body=""):
        ...
        request_body_hash = hashlib.sha256(request_body.encode()).hexdigest().lower()

        StringToSign = (method + ":" + path + ":" + access_token + ":" + request_body_hash + ":" + timestamp).encode()
    ...
    def __getSnapAccessToken(self, timestamp):
        ...
        r = requests.post(self.host + "/openapi/v1.0/access-token/b2b", headers=headers, data=json.dumps(payload), timeout=30)
        return r.json()
    ...
    def getBalance(self, account_number, partnerReferenceNo, external_id=""):
        ...
        signature = self.__getSnapSymetricSignature("POST", path, accessToken, timestamp, data)
        ...
        r = requests.post(self.host + path, headers=header, data=data, timeout=30)
        if self.debug:
